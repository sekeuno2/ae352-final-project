...
class Quadrotor_Sim():
    def __init__(self,
        ...
        gate_scale = 1.
        pos_scale = 0.6
        p1 = pos_scale*np.array([200., 0., 0.])
        p2 = pos_scale*np.array([350., 13., 0.])
        p3 = pos_scale*np.array([400., 30., 0.])
        p4 = pos_scale*np.array([200., 45., 0.])
        p5 = pos_scale*np.array([0., 300., 0.])
        p6 = pos_scale*np.array([0., -160., 0.])
        p7 = pos_scale*np.array([-6., -220., 0.])
        y1 = -np.pi/2.
        y2 = -np.pi/6.
        y3 = 0.
        y4 = np.pi/2.
    ...
    def run(self,
        ...
        while not self.sim.is_done:      
            ...
            orientation = np.array([world_state['roll'],
                                    world_state['pitch'],
                                    world_state['yaw']])
            #Get position of rotors
            # pos, ori = self.bullet_client.getBasePositionAndOrientation(self.sim)
            ...
            c_in_body = np.array([0., self.l, 0.])  
            d_in_body = np.array([0, -self.l, 0])  
            a_in_world = position + a_in_body
            b_in_world = position + b_in_body
            c_in_world = position + c_in_body
            d_in_world = position + d_in_body

            pos_markers = np.concatenate([a_in_world, b_in_world, c_in_world, d_in_world])
